...
public class RunManager : NetworkBehaviour
{
    ...
    public IReadOnlyList<RoomPlan> FloorPlan => floorPlan;
    ...
    private void GenerateFloorPlan(int seed)
    {
        ...
        // 고정 흐름 예시
        // Start -> Battle -> Battle -> Event/Shop/Rest 랜덤 -> Elite -> Rest -> Boss

        AddRoom(RoomType.Start, rng);
        AddRoom(RoomType.Battle, rng);
        AddRoom(RoomType.Battle, rng);

        RoomType midType = GetRandomFrom(rng, RoomType.Event, RoomType.Shop, RoomType.Rest);
        AddRoom(midType, rng);

        AddRoom(RoomType.Elite, rng);
        AddRoom(RoomType.Rest, rng);
    ...
    private RoomType GetRandomFrom(System.Random rng, params RoomType[] types)
    {
