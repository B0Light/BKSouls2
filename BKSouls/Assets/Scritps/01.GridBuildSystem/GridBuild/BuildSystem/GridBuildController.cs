...
public class GridBuildController : MonoBehaviour
{
    ...
    {
        if (buyObject.cost > 0 && WorldPlayerInventory.Instance.balance.Value < buyObject.cost)
            return false;
    ...
    {
        if (buyObject.cost > 0 && !WorldPlayerInventory.Instance.TrySpend(buyObject.cost))
            return false;
